...
public sealed class KeyboardHook: IDisposable {
    ...
    public void Start() {
        if(keyboardHookHandle != 0 || mouseHookHandle != 0) {
            return;
        }
        ...
        AppLog.Write($"Input hooks installed. Keyboard=0x{keyboardHookHandle:X}, mouse=0x{mouseHookHandle:X}.");
    }
    ...
    public void Dispose() {
        ...
        Unhook(ref mouseHookHandle, "mouse");
        Unhook(ref keyboardHookHandle, "keyboard");
        GC.SuppressFinalize(this);
    }

    static void Unhook(ref nint hookHandle, string hookName) {
        if(hookHandle == 0) {
            return;
        }
    ...
    public void Refresh() {
        ...
        var previousKeyboardHandle = keyboardHookHandle;
        var previousMouseHandle = mouseHookHandle;
        keyboardHookHandle = newKeyboardHandle;
        mouseHookHandle = newMouseHandle;
    ...
    nint KeyboardHookCallback(int code, nint wParam, nint lParam) {
        try {
            return ProcessKeyboardCallback(code, wParam, lParam);
        } catch(Exception exception) {
            System.Diagnostics.Debug.WriteLine(
                $"SmartLang keyboard hook callback threw {exception.GetType().Name}: " +
                exception.Message);
            return NativeMethods.CallNextHookEx(keyboardHookHandle, code, wParam, lParam);
        }
    }

    nint ProcessKeyboardCallback(int code, nint wParam, nint lParam) {
        if(code < 0) {
            return NativeMethods.CallNextHookEx(keyboardHookHandle, code, wParam, lParam);
        }
        ...
        var result = engine.Process(unchecked((int)data.VirtualKeyCode), isKeyDown, isInjected);
    ...
    nint MouseHookCallback(int code, nint wParam, nint lParam) {
        try {
            if(code >= 0 && IsPointerInteractionMessage(unchecked((int)wParam))) {
                ...
                var result = engine.ProcessPointerInput(isInjected);
                if(result.ReplayEvents is { Count: > 0 }) {
                    Replay(result.ReplayEvents);
                }
            }
            ...
            System.Diagnostics.Debug.WriteLine(
                $"SmartLang mouse hook callback threw {exception.GetType().Name}: " +
                exception.Message);
        }
    ...
    static void Replay(IReadOnlyList<SyntheticKeyEvent> events) {
            ...
            Data = new NativeMethods.InputUnion {
                Keyboard = new NativeMethods.KeyboardInput {
                    ...
                    Flags =
                        ...
                        (NativeMethods.IsExtendedKey(keyEvent.VirtualKey) ? NativeMethods.KeyeventfExtendedKey : 0u)
                }
            }
        }).ToArray();

        var sent = NativeMethods.SendInput(checked((uint)inputs.Length), inputs, Marshal.SizeOf<NativeMethods.Input>());
